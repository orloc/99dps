...
- **Pet detection** (`internal/gamestate/pet.go`): a pet's generated name (e.g. "Xenab") is learned from `"<Pet> says 'My leader is <character>.'"` (verified against `Tracker.SetCharacter`) or any `"...Master."` command reply; `Tracker.PetName()` exposes it and the Damage meter tags that dealer `(pet)`. **Insta-clickies** with no matchable spell message are handled by a class-gated registry (`internal/gamestate/clicky.go`, `RegisterClicky`).
