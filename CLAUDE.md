...
- **Log-dir resolution** (`resolveLogDir` in `cmd/99dps/main.go`, identical flow on every OS): `-logdir` flag (saved) → `EQ_LOG_DIR` → saved choice (`os.UserConfigDir()/99dps/logdir.txt`) → platform default if it already holds logs → **auto-detect + prompt** (the chosen dir is saved). A debug `-logfile <path>` flag bypasses all of this: it follows one explicit eqlog from the start (so a captured log replays in full), skips the character hot-swap watcher, and does **not** touch the saved log-dir choice — handy for replaying a friend's log. `defaultSpellsPath` then finds `spells_us.txt` next to the log first, falling back to `<logdir>/../spells_us.txt`. Detection (`internal/loader/locate.go` cross-platform core; `scanForEQ`/`eqLogDirFrom`/`logDirFromChoice`) scans candidate roots one level deep for an EQ marker (`eqgame.exe`/`eqclient.ini`/`spells_us.txt` or `eqlog_*.txt`). The platform files supply the roots + prompt UX:
...
- Two simultaneously-alive same-named mobs (`a saltwater croc`) are textually identical — they cannot be told apart. Both the Mob Tracker (one repop entry per kill) and the spell timers (see below) keep *per-instance* state and use heuristics to decide "same mob again" vs "a second same-named mob," since the log itself can't say.
...
- **Segmentation is adaptive-cadence** (`activeForLocked`). Combat *exchanges* — melee `Apply`, `ApplyMagic`, and `ApplySwing` — drive it: a fight rolls to a new session when the gap since the last exchange exceeds `segPulseK × pulse`, clamped to `[segGapFloor, segGapCeil]` seconds, where `pulse` is an EWMA of recent inter-exchange gaps. Dense melee pins the threshold to the floor; slow/sparse caster fights widen it toward the ceiling. **Live-mob rule** (the real boundary is the *kill*, not the clock): a lull past the threshold does *not* split while the engaged mob is still **alive** — i.e. `CombatSession.liveMobs() > 0`: distinct damage-*targets* (mobs) minus credited deaths (`kills`/`xpGains`, "no xp yet ⇒ still up") — *and* this exchange returns to that mob (`reengages`, matched case-insensitively against the session's damage-targets only, so a pet/group-mate dealer can't bridge unrelated fights), within `segLiveCeil` (5 min, the leash/flee bound). So a single pull stays one session through root/med lulls (real example: a drolvarg camp with 29/118/161s gaps, mobs still up); once the mob's **dead** (xp), the next pull — even a same-named respawn — rolls a fresh session, and a *different* mob mid-lull rolls too. `ApplyCrit` and the non-death `ApplyEvent` cases are **annotate-only** (kills/xp/crits are punctuation, never boundaries), so a multi-mob pull with several kills stays one encounter; death and zone/camp are the exception — they force a hard boundary. Constants are tuned against real logs — see their comment in `internal/session/sessionManager.go`.
...
A separate subsystem from DPS: tracks durations of spells the player casts (debuffs on mobs, buffs on self/others). `gamestate.Load` parses the client's `spells_us.txt` (217 `^`-delimited fields; `-spells` flag or `<logdir>/../spells_us.txt`) into a `Book`; `gamestate.Tracker` holds the live timers. The `Tracker` is the package's central stateful object — fed one log line at a time via `Observe` (a deliberate *fan-out*: one line can matter to several subsystems, e.g. a kill both expires the victim's debuffs and records a repop) and read lock-free by the UI. Its state is grouped into composed, caller-locked sub-trackers under one mutex: `cooldownTracker` (reuse/feign/bind, `cooldown.go`), `zoneTracker` (zone/repops/kills, `zone.go`), and `canniMeter` (`canni.go`); spell timers + class/level stay on the core (`tracker.go`). Subsystems that detect the class return it for `inferClassLocked` to apply (a `/who` title always wins). The parser feeds it (via `DmgParser.observeSpells`): `You begin casting X` → pending; a later line that `EndsWith` the spell's `cast_on_other` emote identifies the **target** (the prefix) and starts a timer of `Spell.DurationSeconds(level)` (the EQ buffduration formula). Caster level **and class** come from `/who` self lines (level-ups give level only) via `DmgParser.parseLevel`. EQ `/who` prints a level-based *title* (`[60 Warlord]`), not the class name, so `eqclass.ClassFromTitle` maps the title→class (see `internal/eqclass/class.go`); the `Tracker` stores it (`SetClass`/`Category`). Timers expire on timeout, the target-side `spell_fades` emote, the caster-side `Your <Spell> spell has worn off.` line (the reliable signal for an **early break** — root/mez shaken off, a debuff dropped — `expireSoonestLocked` clears the soonest instance since the line names no target), a resist, or the target being slain. **Same-named mobs are tracked as distinct timers** (mirroring the Mob Tracker's one-entry-per-kill): a landing isn't a blind overwrite of `(spell, target)` — `addOrRefreshTimerLocked` refreshes an existing copy when it's *expiring* (red), and only a **detrimental** spell on a same-named mob can ever spawn a *second* instance (a beneficial buff lands on a unique target — you or a named ally — so a re-cast always refreshes, never duplicates). Urgency is one shared classifier (`gamestate.TimerUrgency`, `internal/gamestate/urgency.go`) used by BOTH the refresh test and the TUI countdown color, so "stale = red" can't drift: green → gold (`lowFrac` 0.5) → red (`StaleFrac` 0.2), but each cutoff is **capped in absolute time** (`expireCapSec` 60s, `lowCapSec` 300s) so a multi-hour self-buff doesn't read orange with an hour left. A second instance gets a unique map key (same `Target`, so grouping/break/dismiss are unaffected). Symmetrically, **one ending event clears one instance**: a fade line drops the soonest same-named copy (not every copy), and a slay drops one instance per spell — so killing or fading one of two `a sand giant` leaves the other's timer running. A re-cast after a copy falls off naturally re-adds it (the freed slot is reused). **Crowd control** (mez + charm + pacify) is flagged on the spell (`Spell.Mez` from the `mesmeriz`/`enthrall` landing emote; `Spell.Charm`; `Spell.Pacify` from the `looks less aggressive` lull/calm/pacify emote) and rendered in a pinned **CROWD CONTROL** section at the top of the timer panel (marked `M`/`⊗`/`z`), above separate DEBUFFS (on mobs) and BUFFS (on you/allies) sections, so a debuff never mixes in with a group-mate's buffs. Pacify uses a P99-specific duration (`Spell.PacifyDurationSeconds` — `DurCap × 6`, no cast-tick: Calm 180s, Pacify 210s; Wake of Tranquility is uncapped ≈ 7 min). Mez and pacify break with no log message when the mob takes damage (it re-aggros), so the parser calls `Tracker.BreakCCOnTarget` on every damage line (names normalized — the landing emote drops the article that the damage line keeps). The tracker is cleared on a character switch. It degrades gracefully (no panel data) if `spells_us.txt` is absent.

**Incoming debuffs (effects on the player)** are a separate, *registry-driven* path (`internal/gamestate/incoming.go`), not derived from the spell book — because an incoming hostile spell logs only its `cast_on_you` emote with **no caster and no caster level**. Two limits follow and shape the design: (1) within an effect family the `cast_on_you` emote is shared (every classic slow → "You feel drowsy."; every root → "Your feet adhere to the ground."), so the emote tells us the **category**, not the spell — we show the category (`Slowed`/`Rooted`/`Snared`/`Crippled`/`Malo'd`/`Tashed`/`Feared`/`Blinded`/`Diseased`/`Poisoned`); (2) the real duration scales with the mob's (unknown) level, so we time the family's **ceiling** (an over-estimate, marked `Timer.Estimated` → a leading `~` in the UI) and clear it early on the authoritative wear-off (`fade`) emote — over-estimate + reliable clear means a timer can linger a beat past the true end but never vanish while you're still debuffed. `incomingDebuffs` is the curated **common melee-relevant** set (each landing emote maps to exactly one category, so detection is unambiguous); `init` builds `incomingByLand`/`incomingByFade`. `matchIncomingDebuffLocked` runs on every line (the detrimental phrasing can't collide with a self-cast, so it's matched regardless of any pending cast) and starts a single per-category timer on `"You"` (re-application refreshes — no same-named instances, since the target is always you); `expireIncomingByFadeLocked` clears it on the fade line, and the existing slay path drops it when you die. These render in their own red **ON YOU** section, pinned above BUFFS in the player-self column (never the Enemy/mob column), and aren't click-dismissible (a debuff on you carries no target to dismiss).
...
A kill landed by **your own pet** (`<mob> has been slain by <petName>!`, where
`petName` is the pet whose `/pet leader` line named you — passed into
`observeLocked`) is credited to **you** (`mine`, killer "You"), not parked under
others' kills; a group-mate's pet stays theirs. The repop list splits **group
kills** (yours, plus a group-mate's that you got xp
...
- **Pet → owner attribution** (`internal/gamestate/pet.go`): the *only* reliable ownership signal is `"<Pet> says 'My leader is <Owner>.'"` (`/pet leader`), so the tracker builds a `petOwners` map from every such line for the **whole group** (`PetOwner(name)`), and flags the player's own pet (`PetName()`, owner == `SetCharacter`). The `"...Master."` command replies are **ignored** — they leak into nearby players' logs, so they can't tell whose pet it is (using them mis-attributed group-mates' pets to you). The Damage meter rolls each pet up under its owner's row (`↳ <pet>`) **and folds the pet's damage into that row's total** — so an owner's Total/DPS/share is inclusive of everything they put out (for the player: melee + spells + pet), with the pet (and `↳ spells`) shown only as dim breakdown children. Ranking and bar-scaling run on these inclusive totals. A pet whose owner dealt no damage is credited as a `<owner>'s pet` row. **Insta-clickies** with no matchable spell message are handled by a class-gated registry (`internal/gamestate/clicky.go`, `RegisterClicky`).
