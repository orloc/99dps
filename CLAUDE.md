...
- **Cooldown timers** sit above the skills breakdown. Abilities like Mend (monk, 360s) emit a distinct self-message and have a fixed reuse, so `spell.cooldownRegistry` matches the activation line (leniently, covering success/partial/fail variants) and the `Tracker` runs a countdown (`Cooldowns()`), rendered green-when-ready / blue-while-counting. Detecting a class-specific ability also infers the class. Add entries to the registry as their exact messages + reuse are confirmed (Feign Death, disciplines, Harm Touch, Lay on Hands are the next candidates).
